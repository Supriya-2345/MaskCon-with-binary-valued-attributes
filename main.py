...
# training config:
parser.add_argument('--dataset', default='cifar100', choices=['CUB'], type=str, help='train dataset')
...
def train(net, data_loader, train_optimizer, epoch, args):
    ...
    for i, [[im_k, im_q], coarse_targets, fine_targets, binary_vectors] in enumerate(train_bar):
        ...
        im_k, im_q, coarse_targets, fine_targets, binary_vectors = im_k.cuda(), im_q.cuda(), coarse_targets.cuda(), fine_targets.cuda(), binary_vectors.cuda()
        if args.mode == 'grafit' or args.mode == 'coins':
            loss = net.forward_explicit(im_k, im_q, coarse_targets,binary_vectors,args)
        else:  # if args.mode == 'maskcon'
            loss = net(im_k, im_q, coarse_targets, binary_vectors, args, epoch)
...
def retrieval(encoder, test_loader, K, chunks=10):
    ...
    with torch.no_grad():
        ...
        for i, (image, _, fine_label,binary_vectors) in enumerate(test_loader):
...
def main_proc(args, model, train_loader, test_loader):
    ...
    """### Start training"""
    # define optimizer
    # optimizer = torch.optim.Adam(model.parameters(), lr=args.lr, weight_decay=args.wd, betas=(0.9, 0.999), eps=1e-08)
    optimizer = torch.optim.SGD(model.parameters(), lr=args.lr, weight_decay=args.wd, momentum=0.7)
    #optimizer = torch.optim.RMSprop(model.parameters(), lr=args.lr, weight_decay=args.wd, alpha=0.99, eps=1e-08, momentum=0)
...
def main():
    ...
    if args.dataset == 'CUB':
        train_dataset = CUB(root=args.data_path, dataset_type='train', train_ratio=0.8, valid_seed=123, transform=DMixTransform([key_transform, query_transform], [1, 1]), target_transform=None)
        test_dataset = CUB(root=args.data_path, dataset_type='test', train_ratio=0.8, valid_seed=123, transform=test_transform, target_transform=None)
        args.num_classes = 11 # Number of Course Classes
        args.size = 224 # image size
